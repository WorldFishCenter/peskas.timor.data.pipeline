...
back by the next job. They are versioned by
[`add_version()`](https://worldfishcenter.github.io/peskas.timor.data.pipeline/reference/add_version.md),
which stamps a timestamp and the commit sha, so every artifact traces to
a unique run:
...
Its shape follows the cross-country template shipped as
...
to `.env` and fill it in for local work —
[`read_config()`](https://worldfishcenter.github.io/peskas.timor.data.pipeline/reference/read_config.md)
loads it through
[`load_dotenv()`](https://worldfishcenter.github.io/peskas.timor.data.pipeline/reference/load_dotenv.md).
In CI the same variables are supplied from GitHub secrets by the
workflow’s `env:` block. `.env` is gitignored and must stay that way.

Note that
[`read_config()`](https://worldfishcenter.github.io/peskas.timor.data.pipeline/reference/read_config.md)
deliberately logs configuration **key names** only. The resolved
configuration holds the service-account key and every token, and GitHub
Actions masks only byte-exact matches of a registered secret.
