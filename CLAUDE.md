...
The five `inst/tinytest/` suites assert against cloud artefacts and run
...
Keep `Dockerfile` and `Dockerfile.prod` in step. **coasts \>= 4.11.0 is
a hard floor.**
...
`.github/workflows/data-pipeline.yaml`, twelve jobs, every 2 days plus
