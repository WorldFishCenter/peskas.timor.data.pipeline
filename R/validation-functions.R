...
# ---------------------------------------------------------------------------
# Ported from `peskas.mozambique.data.pipeline/R/validation-functions.R` in
# migration Phase 5: the shared validation UI writes an approval back to KoBo,
# and a pipeline that overwrote it every run would undo the enumerators' work.
# Neither function exists in `coasts` — see COASTS-TODO C15.

# Both endpoints take either basic auth or a token; Timor uses basic auth,
# because the `KOBO_TOKEN` in its environment belongs to the `peskas` user,
# which has no data access to either Timor asset (measured 2026-08-10: the
# token gets 200 on `/assets/<id>/` and 404 on `/assets/<id>/data/`, while
# `KOBO_USERNAME`/`KOBO_PASSWORD` get 200 on both). Those two are also the only
# KoBo credentials that already exist as GitHub secrets.
kobo_request <- function(url, token = NULL, username = NULL, password = NULL) {
  req <- httr2::request(url) %>%
    # KoBo answers 404 for a submission that has never been validated, which is
    # the normal case; let the status code reach the caller instead of throwing.
  ...
  if (!grepl("^Token ", token)) token <- paste("Token", token)
  httr2::req_headers(req, Authorization = token)
}

kobo_validation_url <- function(asset_id, submission_id) {
  paste0(
    "https://eu.kobotoolbox.org/api/v2/assets/", asset_id, "/data/",
    submission_id, "/validation_status/"
  )
}

#' Read every submission's validation status from a KoBoToolbox asset
#'
#' The bulk form of [get_validation_status()], and what
#' [validate_landings()] uses. The data endpoint returns
#' `_validation_status` alongside `_id` for up to 1,000 submissions per request,
#' so one asset costs `ceiling(n / 1000)` requests rather than one request per
#' submission — measured 2026-08-10, 65 requests and ~90 seconds for v2's 64,997
#' submissions, against more than twenty minutes one at a time. It also covers
#' *every* submission rather than only those a previous run flagged, so an
#' approval entered by hand on a submission the pipeline never flagged is seen
#' too.
#'
...
#' @keywords validation
#' @export
list_validation_statuses <- function(asset_id = NULL,
                                     token = NULL,
                                     username = NULL,
                                     password = NULL,
                                     page_size = 1000) {
  base <- paste0(
    "https://eu.kobotoolbox.org/api/v2/assets/", asset_id, "/data/?",
    "fields=", utils::URLencode('["_id","_validation_status"]', reserved = TRUE),
  ...
  tibble::tibble(
    ...
    validation_status = purrr::map_chr(
      status, ~ .x$uid %||% "not_validated"
    ),
    validated_at = lubridate::as_datetime(purrr::map_dbl(
      status, ~ as.numeric(.x$timestamp %||% NA)
    )),
...
#' Read a submission's validation status from KoBoToolbox
#'
#' [list_validation_statuses()] is the bulk form and the one the pipeline uses;
#' this single-submission accessor exists because
#' [update_validation_status()] re-reads what it wrote.
#'
#' @param submission_id Character. The submission to query.
#' @param asset_id Character. The KoBoToolbox asset id.
#' @param token Character. A KoBoToolbox API token, with or without the
#'   `Token ` prefix. Ignored when `username` is supplied.
#' @param username,password Character. Basic-auth credentials, which is how
#'   Timor authenticates — see the note above [get_validation_status()]'s
#'   definition.
...
#'   transport failure, not an absent status.
#' @keywords validation
#' @export
#'
#' @examples
...
#' )
#' }
get_validation_status <- function(submission_id = NULL,
                                  asset_id = NULL,
                                  token = NULL,
                                  username = NULL,
                                  password = NULL,
                                  debug = FALSE) {
  req <- kobo_request(
    kobo_validation_url(asset_id, submission_id),
    token = token, username = username, password = password
  )
  if (debug) print(req)
  ...
  tryCatch(
    ...
    error = function(e) {
      if (debug) message("Error: ", conditionMessage(e))
      no_status(TRUE)
    }
  )
}

#' Write a submission's validation status back to KoBoToolbox
#'
#' **This mutates the live form.** There is no development KoBoToolbox instance,
#' so the two live assets are the only targets in either environment. It is
#' called by [sync_validation_status()] and by nothing in the recurring
#' pipeline.
#'
#' @inheritParams get_validation_status
...
#'
#' @return A one-row tibble as [get_validation_status()] returns, plus
#'   `update_success`.
#' @keywords validation
#' @export
#'
...
#' )
#' }
update_validation_status <- function(submission_id = NULL,
                                     asset_id = NULL,
                                     token = NULL,
                                     username = NULL,
                                     password = NULL,
