...
#'
#' This function get Pelagic Data System API responses and return them as a
#' data frame. It is used inside ingest functions in order to process multiple
#' API responses.
#'
#' @param data A character indicating either "trips" or "tracks"
#' @param secret Access secret code for the account
#' @param token Access token for the account
#' @param id Trip unique id from which retrieve tracks data
#'
#' @return A data frame with tracks or trips data
#' @export
#'
#' @examples
#' \dontrun{
#'
#'   get trips data:
#'   get_pds_resp("trips",secret = "abcXXXXXX", token = "123XXXXXX")
#'
#'   get tracks data:
#'   get_pds_resp("tracks",secret = "abcXXXXXX",token = "123XXXXXX", id="2327722")
#' }
#'
get_pds_resp <- function(data, secret = NULL, token = NULL, id=NULL){

  if (data=="trips"){
    request_url <- paste("https://analytics.pelagicdata.com/api",token,
                         "v1/trips",start_date= "2018-07-01", end_date=Sys.Date(),
                         "?deviceInfo=true", sep = "/")
    ...
    readr::read_csv(httr::content(res,"text"),
