...
# the same source `estimate_fishery_indicators()` uses. The Google Sheets
# `fishing_vessel_statistics` table this used to summarise was dropped in
# migration Phase 11.
...
```{r, echo=FALSE}
# Frozen content: `ingest_kepler_tracks()` wrote this object and was deleted in
# migration Phase 11 (no workflow had called it since 2021). The object is still
# in the bucket, so the iframe renders, but it is not regenerated.
