...
# get data
  ...
  peskas.timor.data.pipeline::get_tracks_map(pars)
  timor_map <- list.files(pattern = "tracks-map", full.names = TRUE)
...
trips %>%
  ...
  labs(
    ...
    subtitle = "Monthly number of non-unique fishers in Timor,\nmale adult fishers are by far the most representative.",
    fill = ""
  ) +
  scale_x_date(date_breaks = "3 month", minor_breaks = NULL, labels = label_date,
               limits = c(as.Date("2017-07-01"),Sys.Date() - 31)) +
...
trips %>% 
  ...
  labs(
    ...
    subtitle = "Atauro municpality recorded most of the trips.\nSeveral municipalities have joined the project over time."
  ) +
  scale_x_date(date_breaks = "3 month", minor_breaks = NULL, labels = label_date,
               limits = c(as.Date("2017-07-01"),Sys.Date() - 31)) +
  theme_minimal(15) +
...
# Boat tracks
```{r, echo=FALSE, message=FALSE, fig.height=8, fig.width=6, fig.align='left',fig.asp=2/3}
knitr::include_graphics(timor_map)
```
\pagebreak
