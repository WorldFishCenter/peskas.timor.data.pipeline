...
kobo_trips <- peskas.timor.data.pipeline::get_merged_trips(pars) %>%
...
# Short labels for the model's catch groups; the catch composition table
# expects exactly these, in its icon order. Other taxa keep their FAO name.
group_labels <- c(
  CLP = "Sardines/pilchards", GZP = "Garfish", TUN = "Tuna/Bonito/Other Mackerel",
  SDX = "Mackerel scad", FLY = "Flying fish", SNA = "Snapper/seaperch",
  MOO = "Moonfish", CGX = "Jacks/Trevally/Other Scad", CJX = "Fusilier",
  BEN = "Long tom", RAX = "Short bodied mackerel", LWX = "Jobfish", MZZ = "Other"
)
catch_types <-
  peskas.timor.data.pipeline::get_assets(pars)$taxa %>%
  dplyr::distinct(interagency_code = .data$alpha3_code, catch_name_en = .data$english_name) %>%
  dplyr::mutate(catch_name_en = dplyr::coalesce(unname(group_labels[interagency_code]), catch_name_en))

vessels_stats <- peskas.timor.data.pipeline:::get_registered_boats(pars)
...
label_date <- function(x) {
  dplyr::case_when(
    lubridate::month(x) == 1 ~ format(x, "%Y"),
    TRUE ~ format(x, "%b")
  )
}
...
kobo_trips %>%
  ...
  scale_fill_gradientn(
    colours = c("white", "#a4c3c8", "#7fa5a9", "#35666f", "#103235")
  ) +
...
kobo_trips %>%
  ...
  labs(
    x = "Date",
    y = "Catch per unit effort (kg / fisher-hour)",
...
estimates$national$aggregated %>%
  ggplot(aes(as.Date(landing_period), n_landings_per_boat)) +
  geom_line(color = "#447597") +
  geom_point(color = "#447597") +
...
estimates$national$aggregated %>%
  ggplot(aes(as.Date(landing_period), catch / 1000)) +
  geom_line(color = "#447597") +
  geom_point(color = "#447597") +
  geom_area(fill = "#447597", alpha = 0.25) +
  # geom_area(fill="#447597",alpha=0.15)+
...
estimates$national$aggregated %>%
  ggplot(aes(as.Date(landing_period), catch_price)) +
  geom_line(color = "#447597") +
  geom_point(color = "#447597") +
  geom_area(fill = "#447597", alpha = 0.25) +
  # geom_area(fill="#447597",alpha=0.15)+
...
estimates$national$aggregated %>%
  ggplot(aes(as.Date(landing_period), price_kg)) +
  geom_line(color = "#447597") +
  geom_point(color = "#447597") +
  geom_area(fill = "#447597", alpha = 0.25) +
  # geom_area(fill="#447597",alpha=0.15)+
...
species <-
  catch_types %>%
  dplyr::select(catch_taxon = interagency_code, catch_name_en)
...
preplot %>%
  ...
  ggplot(aes(
    x = reorder(catch_name_en, tot_obs),
    y = prop_obs,
    fill = catch_use
  )) +
...
preplot %>%
  ...
  #dplyr::left_join(species, by = "catch_taxon") %>%
  ggplot(aes(
    x = reorder(municipality, tot_obs),
    y = prop_obs,
    fill = catch_use
  )) +
  ggchicklet::geom_chicklet() +
...
estimates$national$taxa %>%
  ...
  ggplot(aes(x = as.Date(landing_period), y = catch / 1000)) +
...
species <-
  catch_types %>%
  dplyr::select(grouped_taxa = interagency_code, catch_name_en)
...
estimates$municipal %>%
  ...
  ggplot(aes(
    x = reorder(reporting_region, total_catch),
    y = percent_catch,
    fill = catch_name_en
  )) +
  #geom_col() +
...
dat <-
  ...
  dplyr::filter(
    !is.na(fisher_number_child),
    !is.na(fisher_number_woman),
    !is.na(fisher_number_man)
  )
...
parallel_plot <-
  ...
  dplyr::mutate(
    name = dplyr::case_when(
      name == "fisher_number_child" ~ "Childs",
      name == "fisher_number_woman" ~ "Women",
      name == "fisher_number_man" ~ "Men"
    ),
    name = as.factor(name)
  ) %>%
  dplyr::rename("Gender" = name, "Habitat" = habitat, "Gear type" = gear) %>%
  ggforce::gather_set_data(c(3, 1, 2))

parallel_plot$x <- factor(
  parallel_plot$x,
  levels = c("3", "1", "2"),
  labels = c("Gender", "Habitat", "Gear type")
)

# `y` stacks the Gender, Habitat and Gear values into one column, so these
# levels are the union of all three and only control the axis ordering.
#
# The nine gear labels were lowercase ("gill net", "manual collection", ...)
# until 2026-09-07 and every one resolved to NA, because `preprocess_landings()`
# sets `gear` from the PESKAS | FRAME `standard_name`, which is Title Case
# ("Gill Net", "Gleaning", ...). `na.omit()` below then dropped those rows, so
# the plot lost its Gear axis entirely. Two were also wrong on wording:
# "seine net" is `Seine` and "manual collection" is `Gleaning`.
#
# Any label the frame or the habitat table adds later is appended rather than
# silently dropped, so this cannot fail the same way twice.
y_levels <- c(
  "Men",
  "Childs",
  "Women",
...
y_levels <- c(
  y_levels,
  setdiff(unique(as.character(parallel_plot$y)), y_levels)
)

parallel_plot$y <- factor(parallel_plot$y, levels = y_levels)
...
p1 <-
  ...
  dplyr::mutate(
    category = dplyr::case_when(
      fisher_number_man == 0 ~ "Women and childs",
      TRUE ~ "Only men"
    )
  ) %>%
  ...
  na.omit() %>%
  dplyr::mutate(n_tot = sum(n, na.rm = T), n = round(n / n_tot * 100, 2)) %>%
  ggplot(aes(
    area = n,
    fill = habitat,
  ...
  treemapify::geom_treemap_text(colour = "black", place = "centre", size = 13) +
...
ggplot2::ggplot() +
  ...
  ggplot2::labs(
    x = "",
    y = "Matched RNI from 100g portion",
    fill = "Nutrient"
  ) +
  ggplot2::theme(
    legend.position = "bottom",
...
```{r echo=FALSE, fig.height=5.5, fig.width=7, message=FALSE, warning=FALSE, out.height="100%",out.width="100%"}
