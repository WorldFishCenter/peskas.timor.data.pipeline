...
estimates$municipal %>%
  ...
  dplyr::left_join(vessels_stats, by = "municipality") %>%
  ggplot(aes(landing_period, catch / 1000)) +
  facet_wrap(~municipality, ncol = 3, scales = "free") +
  geom_line(color = "#447597") +
...
preplot <-
  ...
  dplyr::filter(!catch_taxon %in% ("0")) %>%
  dplyr::group_by(.data$municipality, catch_purpose) %>%
  dplyr::summarise(
    obs = dplyr::n()
  ) %>%
  dplyr::group_by(.data$municipality) %>%
...
preplot %>% 
  dplyr::filter(!is.na(municipality)) %>%
  #dplyr::left_join(species, by = "catch_taxon") %>% 
  ggplot(aes(x = reorder(municipality, tot_obs), y = prop_obs, fill = catch_purpose)) +
  ggchicklet::geom_chicklet() +
  theme_minimal(9) +
  theme(
    legend.position = "bottom",
...
estimates$municipal %>%
  ...
  dplyr::bind_rows(.id = "municipality") %>%
  dplyr::left_join(vessels_stats, by = "municipality") %>% 
  dplyr::group_by(municipality, grouped_taxa) %>%
  dplyr::summarise(catch = sum(catch, na.rm = T)) %>%
  dplyr::ungroup() %>% 
  dplyr::left_join(species, by = "grouped_taxa") %>% 
  dplyr::select(-grouped_taxa) %>%
  tidyr::complete(municipality, catch_name_en, fill = list(catch = 0)) %>%
  ...
  ggplot(aes(x = reorder(municipality, total_catch), y = percent_catch, fill = catch_name_en)) +
  #geom_col() +
  ggchicklet::geom_chicklet() +
  theme_minimal() +
  theme(
