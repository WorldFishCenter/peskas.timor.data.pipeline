...
estimates$national$aggregated %>%
  dplyr::mutate(n = n_landings_per_boat * 2334) %>%
...
trips %>%
    ...
    dplyr::summarise(monthly_effort=sum(landing_survey_trip_duration,na.rm=TRUE),
                     trips = dplyr::n()) %>%
     na.omit() %>%
...
estimates$national$aggregated %>%
  dplyr::mutate(pk = revenue/catch) %>% 
  ggplot(aes(landing_period, pk))+
  geom_line(color = "#447597") +
  geom_point(color = "#447597") +
  geom_area(fill="#447597",alpha=0.15)+
  stat_smooth(method = "loess", color = "#976644", alpha = 0.1,size=0.5) +
...
trips %>% 
  ...
  dplyr::summarise(total_weight = dplyr::first(total_weight),
...
estimates$national$taxa %>% 
  ...
  dplyr::mutate(comm_name = dplyr::case_when(comm_name == "Sardines/pilchards" ~ "Sardines",
...
p1 <-
  dplyr::left_join(catch,trips, by="trip_id") %>%
  dplyr::filter(individuals>0) %>%
  dplyr::mutate(catch_taxon = dplyr::case_when(catch_taxon %in% pars$models$modelled_taxa ~ catch_taxon,
                                               TRUE ~ "MZZ"),
                comm_name = dplyr::case_when(catch_taxon == "MZZ" ~ "Other",
                                             TRUE ~ comm_name)) %>% 
  ...
  scale_x_continuous(labels=scales::percent,
                     expand = c(0,0))+
  theme_minimal(15)+
  labs(x="Overall catch composition",
       y="",
       title="")+
