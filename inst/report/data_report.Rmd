...
gear_plot <-
  tidyr::tibble(
    gear_type = metadata$gear_types$gear_id,
    n_boats = purrr::map_dbl(metadata$gear_types$boats, length)
  ) %>%
  dplyr::filter(gear_type != "manual collection") %>%
  dplyr::mutate(
  ...
  ggplot(aes(x = dummy_var, y = n_boats, fill = reorder(gear_type, n_boats))) +
  geom_col(width = 2) +
  ...
  labs(
    ...
    subtitle = "The fleet associated to Peskas is composed of a total of 451 boats, most of them  motorised.\nMore than 90% of the boats are equipped with hand line and long line while about 6% equip spear guns."
  ) +
  scale_fill_manual(values = rev(c("#3d405b", "#6f1a07", "#81b29a", "#96c5f7", "#f2cc8f", "#e07a5f", "#ffd23f", "#625834"))) +
  coord_flip()


vessel_plot <-
  tidyr::tibble(
    boat_type = metadata$vessel_types$boat_type,
    n_boats = purrr::map_dbl(metadata$vessel_types$boats, length)
  ) %>%
  dplyr::filter(.data$boat_type != "gleaning") %>%
  dplyr::mutate(
    dummy_var = rep("dummycol", nrow(.)),
    total_boats = sum(.data$n_boats),
    proportion = .data$n_boats / .data$total_boats * 100,
    proportion_label = dplyr::case_when(
      .data$proportion < 5 ~ NA_real_,
      TRUE ~ .data$proportion
    )
  ) %>%
