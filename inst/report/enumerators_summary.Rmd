...
**The following report summarizes the peskAAS survey activity in each landing site during the last month `r paste(last_month_lab, last_year)` and the last year `r last_year`. The purpose of this report is to provide an overview of the number of surveys submitted, the number of working days, and the estimated working hours per day.**
...
The table below shows the overall activity in each landing site referred to the latest month r `target_month_lab` compared to the previous one `r ref_month_lab`. Colours and numbers in brackets in each cell refer to the percentage variation of each activity indicator from the previous month. Grey cells indicate no possible comparison due to lack of data.
