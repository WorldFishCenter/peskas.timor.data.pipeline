...
**The following report summarizes the peskAAS survey activity in each
landing site during the last month `r paste(last_month_lab, last_year)`
and the last year `r last_year`. The purpose of this report is to
provide an overview of the number of surveys submitted, the number of
working days, and the estimated working hours per day.**
...
# setwd("../..")
setwd("/Users/lore/My Drive/WorldFish/peskas.timor.data.pipeline")
...
landings_raw <-
  ...
  dplyr::select(
    ...
    n_boats = no_boats,
    reason_zero_boats = Tanba_sa_la_iha_ro_o_peskador_,
    date,
    today,
    start,
    end
...
landings <-
  ...
  dplyr::filter(Year >= last_year) %>%
  dplyr::select(landing_id, landing_date, landing_station) %>%
  dplyr::mutate(
    landing_station = trimws(.data$landing_station, which = "both"),
    landing_station = dplyr::case_when(
      landing_station == "Oenunu/Citrana/Bona/Baoknana" ~ "Oenunu/Citrana",
...
The graph below show the monthly activity in the different sites in
`r this_year` where each cells represent a calendar day. Cell colors
indicate the number of submissions. The vertical axis is ranked by the
most active landing sites.
...
The graph below show the number of days of activity in each month in
each site. A day of activity means that in that specific day at least 1
submission was recorded. The black vertical line in each panel indicates
the average of the days of activity, the number in the bars indicates
the number of submissions for that specific month.
...
```{r message=FALSE, warning=FALSE, include=FALSE}
target_month <- lubridate::floor_date(Sys.Date() - 30, "month")
ref_month <- lubridate::floor_date(target_month - 30, "month")

target_month_lab <- lubridate::month(target_month, label = T, abbr = F)
ref_month_lab <- lubridate::month(ref_month, label = T, abbr = F)
```

The table below shows the overall activity in each landing site referred
to the latest month r `target_month_lab` compared to the previous one
`r ref_month_lab`. Colours and numbers in brackets in each cell refer to
the percentage variation of each activity indicator from the previous
month. Grey cells indicate no possible comparison due to lack of data.
...
df_tab <- 
  ...
  dplyr::group_by(month, landing_station) %>% 
  dplyr::summarise(N.submissions = sum(N.submissions),
                   `Days of activity` = sum(`Days of activity`)) %>% 
  dplyr::mutate(`Daily work hours` = ((N.submissions * 6) / 60) /220,
  ...
  dplyr::mutate(diff_nsub = ((nsub_now - nsub_before)/nsub_before) * 100,
...
df_tab_cols <- 
  ...
  dplyr::rename(days_now = diff_days,
                nsub_now = diff_nsub,
                wh_now = diff_wh) %>% 
