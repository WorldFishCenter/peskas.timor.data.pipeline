...
catch_types <- peskas.timor.data.pipeline::get_catch_types(pars) %>% 
  dplyr::select('ASFIS code'=interagency_code,
                'Taxonomic rank (family)'=catch_family,
                'Common name'=catch_name_en)
}
...
(ref:overall-prop) Overall proportion of economic value and weight by each catch type. 

```{r overall-prop, echo=FALSE, warning=FALSE, fig.cap='(ref:overall-prop)', fig.height=7, fig.width=6,out.height="100%",out.width="100%"}

revenue_df <-
  dplyr::left_join(catch, trips, by = "trip_id") %>%
  dplyr::filter(individuals > 0 & nchar(landing_taxa) == 3) %>%
  dplyr::group_by(landing_taxa) %>%
...
dplyr::left_join(revenue_df, weight_df, by = "landing_taxa") %>%
  dplyr::select(landing_taxa, value_proportion, weight_proportion) %>%
  dplyr::arrange(value_proportion) %>%
  dplyr::mutate(landing_taxa = factor(landing_taxa, levels = .$landing_taxa)) %>%
  tidyr::pivot_longer(-landing_taxa) %>%
  ggplot(aes(value, landing_taxa)) +
  geom_line(aes(group = landing_taxa), color = "#b2b2b2") +
  geom_point(aes(color = name),size=2.5) +
  theme_minimal(12) +
  scale_x_continuous(labels = scales::percent_format(scale = 1)) +
  scale_color_manual(labels = c("Economic value", "Weight"), values = c("#859e74", "#8d749e")) +
