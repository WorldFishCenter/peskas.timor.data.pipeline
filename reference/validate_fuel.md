...
Alert 23 never reaches a flag string: fuel is one of the three frames
that feed the validated artefact but not the flag composition in
[`validate_landings()`](https://worldfishcenter.github.io/peskas.timor.data.pipeline/reference/validate_landings.md).

## Usage

``` r
validate_fuel(submissions, method, k_fuel)
```
...
- k_fuel:
  ...
  Extension of bounds for the fuel volume.

## Value

A tibble: `submission_id`, `fuel`, `alert_number`.
