...
Projects the long validated catch table written by
[`validate_landings()`](https://worldfishcenter.github.io/peskas.timor.data.pipeline/reference/validate_landings.md)
onto the 22-column schema `peskas-api-{dev,prod}` publishes for every
country, and uploads it to `conf$api$trips$validated$cloud_path`. Same
schema as
[`export_api_raw()`](https://worldfishcenter.github.io/peskas.timor.data.pipeline/reference/export_api_raw.md);
the two differ only in their input.
